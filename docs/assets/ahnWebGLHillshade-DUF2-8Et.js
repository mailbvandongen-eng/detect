import{B as M,L as w,W as D,e as f,n as H,u as P,U as a,P as W,A as L,C as G,N as b,g as F,c as T,X as N}from"./index-Dug-D3ed.js";function O(t,e){const r=`
  ...
  `,n={...H(),bandCount:e},s=[];if(t.color!==void 0){const o=f(n,t.color,G);s.push(`color = ${o};`)}if(t.contrast!==void 0){const o=f(n,t.contrast,b);s.push(`color.rgb = clamp((${o} + 1.0) * color.rgb - (${o} / 2.0), vec3(0.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0));`)}if(t.exposure!==void 0){const o=f(n,t.exposure,b);s.push(`color.rgb = clamp((${o} + 1.0) * color.rgb, vec3(0.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0));`)}if(t.saturation!==void 0){const o=f(n,t.saturation,b);s.push(`
    ...
    `)}if(t.gamma!==void 0){const o=f(n,t.gamma,b);s.push(`color.rgb = pow(color.rgb, vec3(1.0 / ${o}));`)}if(t.brightness!==void 0){const o=f(n,t.brightness,b);s.push(`color.rgb = clamp(color.rgb + ${o}, vec3(0.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0));`)}const l={},u=Object.keys(n.variables).length;if(u>1&&!t.variables)throw new Error(`Missing variables in style (expected ${n.variables})`);for(let o=0;o<u;++o){const m=n.variables[Object.keys(n.variables)[o]];if(!(m.name in t.variables))throw new Error(`Missing '${m.name}' in style variables`);const h=P(m.name);l[h]=function(){let g=t.variables[m.name];return typeof g=="string"&&(g=F(g)),g!==void 0?g:-9999999}}const i=Object.keys(l).map(function(o){return`uniform float ${o};`}),v=Math.ceil(e/4);i.push(`uniform sampler2D ${a.TILE_TEXTURE_ARRAY}[${v}];`),n.paletteTextures&&i.push(`uniform sampler2D ${W}[${n.paletteTextures.length}];`);const d=Object.keys(n.functions).map(function(o){return n.functions[o]}),E=`
