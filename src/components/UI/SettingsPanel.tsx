      ...
      >
        <div className="p-3 space-y-5">
          {activeTab === 'algemeen' && <>
            ...
            <section className="space-y-2"><h3 className="font-medium text-gray-800 flex items-center gap-2"><Smartphone size={16} /> Weergave</h3><label className="flex items-center justify-between gap-3 text-sm"><span>Kleurmodus</span><select value={settings.colorScheme} onChange={e => settings.setColorScheme(e.target.value as ColorSchemePreference)}><option value="system">Systeem</option><option value="light">Licht</option><option value="dark">Donker</option></select></label><p className="text-xs text-gray-500">Systeem volgt automatisch de lichte of donkere modus van je telefoon.</p></section>
            <section className="space-y-2"><h3 className="font-medium text-gray-800 flex items-center gap-2"><Map size={16} /> Kaart</h3><label className="flex items-center justify-between text-sm"><span>Schaalbalk</span><input type="checkbox" checked={settings.showScaleBar} onChange={e => settings.setShowScaleBar(e.target.checked)} /></label><label className="flex items-center justify-between text-sm"><span>Standaardkaart</span><select value={settings.defaultBackground} onChange={e => settings.setDefaultBackground(e.target.value as DefaultBackground)}><option>Esri (licht)</option><option>Luchtfoto</option><option>OpenStreetMap</option><option>Hybride (wereld)</option></select></label></section>
